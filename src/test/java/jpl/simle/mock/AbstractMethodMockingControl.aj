...
public abstract aspect AbstractMethodMockingControl percflow(mockStaticsTestMethod()) {
	...
	after() : mockStaticsTestMethod() {
		//System.out.println("Verifying; after: " + thisJoinPointStaticPart);
	...
	Object around() : methodToMock() {
		if (recording) {
			//System.out.println("Recording: " + lastSig);
			expectations.expectCall(thisJoinPointStaticPart.toLongString(), thisJoinPoint.getArgs());
			// Return value doesn't matter
			return null;
		} else {
			//System.out.println("Replay: " + thisEnclosingJoinPointStaticPart.toLongString());
